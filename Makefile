...
conda-dev: ## conda update development dependencies
	conda env update -n neutron_analysis-env -f environment-dev.yml
...
mamba-dev: ## mamba update development dependencies
	mamba env update -n neutron_analysis-env -f environment-dev.yml

mamba-all: mamba-env mamba-dev ## mamba create development env

activate: ## activate base env
	conda activate neutron_analysis-env
...
user-venv: ## create .venv file with name of conda env
	echo neutron_analysis-env > .venv

user-autoenv-zsh: ## create .autoenv.zsh files
	echo conda activate neutron_analysis-env > .autoenv.zsh
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source neutron_analysis -m pytest
	coverage report -m
	coverage html
...
docs: ## generate Sphinx HTML documentation, including API docs
	rm -f docs/neutron_analysis.rst
...
conda-dist: ## build conda dist (run dist and clean?)
	mkdir conda_dist; \
	cd cond_dist; \
	grayskull pypi neutron_analysis ; \
	conda-build ; \
	echo 'upload now'
